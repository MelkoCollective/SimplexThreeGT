function Configurations.from_dict(::Type{AnnealingTask}, ::Type{TOMLRange}, x::AbstractDict{String})
    TOMLRange(x["start"], x["stop"], x["step"])
end

function Configurations.to_dict(::Type{AnnealingTask}, x::TOMLRange)
