...
function write_checkpoint(io::IO, temp::Float64, spins::BitVector)
    write(io, temp)
...
function read_checkpoint(io::IO, nspins::Int)
    return read(io, Float64), read_spins(io, nspins)
end

function find_checkpoint(io::IO, T::Float64, nspins::Int)
    d = find_checkpoint(io, [T], nspins)
    return (first(d)..., )
end

function find_checkpoint(io::IO, temps, nspins::Int)
